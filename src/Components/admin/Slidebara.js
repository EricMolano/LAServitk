...
import Profile from './ProfileA'; // Import Profile component
...
const Slidebara = () => {
  ...
  return (
    ...
    >
      ...
      <div className="lista-items">
        ...
        {isOpen && isProductosOpen && (
          <motion.div 
            ...
            transition={{ duration: 0.3 }}
          >
            <Link to="/Inventory" className="btn-cerrar-sesion">
              <div className="icono-item"><FaBox /></div>
              {isOpen && <div className="texto-enlace-item">Inventario</div>}
            </Link>
            <Link to="/VerSolicitudes" className="btn-cerrar-sesion">
      ...
      {isProfileOpen && <Profile showModal={isProfileOpen} closeModal={toggleProfile} />}
    </motion.div>
  );
};

export default Slidebara;